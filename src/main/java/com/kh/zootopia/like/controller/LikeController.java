...
public class LikeController {
	 ...
	 */
	public ModelAndView checkLike(@ModelAttribute Like like, String viewAddress, ModelAndView mv) {
		...
		try {
			...
			if (result > 0) {
				...
				mv.addObject("like", 1).setViewName(""); // 1이 들어가면 좋아요가 클릭된 상태로 표시, 좋아요 한 번 더 클릭 시 취소
				
			} else {
				
				mv.addObject("like", 0).setViewName(""); // 0이 들어가면 좋아요 버튼을 클릭할 수 있도록 활성화 
